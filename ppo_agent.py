...
class PPOAgent:
    def __init__(self, state_size=13, action_size=3):
        ...
        self.actor_optim = tf.keras.optimizers.Adam(self.learning_rate)
        self.critic_optim = tf.keras.optimizers.Adam(self.learning_rate)
    ...
    def train(self):
        ...
        next_vals = self.critic.predict(next_states).flatten()
        ...
        for t in reversed(range(len(rewards))):
            delta = rewards[t] + self.gamma * next_vals[t] * (1 - dones[t]) - values[t]
        ...
        with tf.GradientTape() as tape:
            ...
            clipped_ratios = tf.clip_by_value(ratios, 1 - self.eps, 1 + self.eps)
        ...
        self.actor_optim.apply_gradients(zip(actor_grads, self.actor.model.trainable_variables))
